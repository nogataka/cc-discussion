 ...
 * ==================
 *
 * Main CLI module for the cc-discussion npm global package.
 * Handles Python detection, virtual environment management,
 * config loading, and uvicorn server lifecycle.
 *
 * Based on AutoForge CLI - uses only Node.js built-in modules.
 */
...
/** Root of the cc-discussion npm package (one level up from lib/) */
const PKG_DIR = dirname(dirname(fileURLToPath(import.meta.url)));

/** User config home: ~/.cc-discussion/ */
const CONFIG_HOME = join(homedir(), '.cc-discussion');
...
const REQUIREMENTS_FILE = join(PKG_DIR, 'requirements.txt');
 ...
 *
 * The CC_DISCUSSION_PYTHON env var overrides automatic detection.
 */
function findPython() {
    // Allow explicit override via environment variable
    const override = process.env.CC_DISCUSSION_PYTHON;
    if (override) {
        const result = tryPythonCandidate(override);
        if (!result) {
            die(`CC_DISCUSSION_PYTHON is set to "${override}" but it could not be executed.`);
        }
        if (result.tooOld) {
            die(
                `Python ${result.version.raw} found (via CC_DISCUSSION_PYTHON), but 3.11+ required.\n` +
...
/** SHA-256 hash of the requirements.txt file contents. */
...
function ensureVenv(python, forceRecreate) {
    ...
    if (needsCreate) {
        ...
        log(`      Creating virtual environment at ~/.cc-discussion/venv/`);
...
/**
 * Ensure ~/.cc-discussion/.env exists. On first run, copy .env.example
...
function ensureEnvFile() {
    ...
    if (existsSync(ENV_EXAMPLE)) {
        ...
        writeFileSync(ENV_FILE, '# CC-Discussion configuration\n# See documentation for available options.\n', 'utf8');
    }
    return true;
}
...
function printVersion() {
    console.log(`cc-discussion v${VERSION}`);
}

function printHelp() {
    console.log(`
  CC-Discussion v${VERSION}
  Multi-Claude/Codex discussion platform

  Usage:
    cc-discussion                    Start the server (default)
    cc-discussion config             Open ~/.cc-discussion/.env in $EDITOR
    cc-discussion config --path      Print config file path
    cc-discussion config --show      Show effective configuration
...
function startServer(opts) {
    ...
    let python;
    if (fastPath) {
        // Skip the Python search header on fast path
        python = null;
    } else {
    ...
    if (existingPid && isProcessAlive(existingPid)) {
        log(`CC-Discussion is already running at http://${host}:${port}`);
        log('Opening browser...');
    ...
    if (configCreated) {
        log(`      Created config file: ~/.cc-discussion/.env`);
        log('');
    }
    ...
    if (host !== '127.0.0.1') {
        ...
        console.log('  The CC-Discussion UI will be accessible from other machines.');
